...
URL = "https://www.nietbschool.ac.in/prati-dhwani"
OUTPUT_FILE = Path("../output/club_prati_dhwani.json")
...
def scrape_prati_dhwani():
    ...
    if not main_section:
        raise RuntimeError("Prati Dhwani content section not found")
    ...
    if text_container:
        for p in text_container.find_all("p"):
            description_blocks.append(p.get_text(" ", strip=True))
...
def main():
    ...
    data = scrape_prati_dhwani()

    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=4, ensure_ascii=False)

    print("✅ Prati Dhwani club data scraped successfully")
