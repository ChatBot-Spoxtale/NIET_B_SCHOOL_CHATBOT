...
export default function NIETChatbotMessages({embed=false}) {
  ...
  const handleOptionClick = (opt, messageId) => {
    if (opt === "Apply Now") {
      window.open("https://www.nietbschool.ac.in/apply-now", "_blank")
      return
    }
    ...
    if (opt === "About NIET B SCHOOL") {
      sendMessage("About NIET B SCHOOL")
      return
    }
  ...
  >
      ...
      <div className="px-5 py-4 bg-gradient-to-r from-[#e2111f] via-[#d00a1c] to-[#b00d18] flex items-center gap-3 shrink-0 shadow-lg relative z-10 border-b border-white/10 overflow-hidden">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            <span className="flex h-1.5 w-1.5 rounded-full bg-lime-300 animate-pulse shadow-[0_0_6px_rgba(132,204,22,0.5)]" />
            <span className="text-[14px] font-semibold text-white/80 uppercase tracking-wider">Always online</span>
          </div>
        </div>
        ...
        <button
          onClick={() => {
            ...
            setCallbackData({ name: "", phone: "" })
            sessionStorage.removeItem("niet_chat_messages")
            pushBot("Hello! I'm the NIET B SCHOOL Assistant — how can I help you today?")
            pushOptions(INITIAL_OPTIONS, false)
          }}
          className="p-2 hover:bg-white/15 rounded-lg transition-all duration-200 active:scale-95 z-20"
          title="Reset View"
        >
          <svg
      ...
      <div
        ref={messagesRef}
        className="flex-1 overflow-y-auto px-4 py-5 flex flex-col gap-3 no-overscroll relative z-0 bg-white"
      >
        {messages.map((m, idx) => (
          <div
            key={m.id}
          ...
          >
            ...
            {m.from === "user" && (
              <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-[#e2111f] to-[#b00d18] flex items-center justify-center text-white font-bold shrink-0 mt-1 shadow-sm">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {m.options.map((opt) => (
                          <button
                            ...
                            className={`option-chip ${m.selectedValue === opt ? "option-chip-active" : ""}`}
                          >
                            {opt}
                            {opt === "Apply Now" && (
        ...
        {typing && (
          <div className="flex gap-2 items-center ml-1 mt-2">
            <div className="flex items-center gap-1">
              <div className="typing-dot animate-[typing_1.2s_infinite]" style={{ animationDelay: "0s" }} />
              <div className="typing-dot animate-[typing_1.2s_infinite]" style={{ animationDelay: "0.12s" }} />
              <div className="typing-dot animate-[typing_1.2s_infinite]" style={{ animationDelay: "0.24s" }} />
            </div>
          </div>
        )}
          ...
          <div className="px-5 pb-3">
            <div className="suggestion-bar flex gap-2 flex-wrap">
              {suggestions.map((s) => (
                <button
                  key={s}
                  onClick={() => handleOptionClick(s, lastOptionsId)}
      ...
      <div className={`px-4 py-4 ${embed ? "" : "bg-white border-t border-slate-100/50 shadow-[0_-4px_16px_rgba(0,0,0,0.08)]"}`}>
        ...
        >
          <input
            ...
            className={`w-full ${embed ? "bg-transparent border-slate-400/50" : "bg-gradient-to-r from-slate-50 to-slate-100 border-slate-200"} rounded-full px-5 py-3 pr-14 text-[15px] text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#e2111f]/40 focus:border-transparent focus:bg-white transition-all duration-300 placeholder:text-slate-500 font-medium h-12 shadow-sm border`}
            placeholder={callbackStep ? `Enter your ${callbackStep}...` : "Ask me anything..."}
          />
          <button
            disabled={isSending || !input.trim()}
            className="absolute right-1.5 w-8 h-8 rounded-full bg-gradient-to-br from-[#e2111f] to-[#b00d18] flex items-center justify-center text-white shadow-lg transition-all duration-300 hover:scale-115 active:scale-90 disabled:opacity-30 disabled:scale-100 disabled:shadow-none"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
